#ifndef FILE_PATH_H_
#define FILE_PATH_H_

#define FILE_NUM 10

const char * file_paths[FILE_NUM] = {
   "../tra_data/tra_data0.txt",
   "../tra_data/tra_data1.txt",
   "../tra_data/tra_data2.txt",
   "../tra_data/tra_data3.txt",
   "../tra_data/tra_data4.txt",
   "../tra_data/tra_data5.txt",
   "../tra_data/tra_data6.txt",
   "../tra_data/tra_data7.txt",
   "../tra_data/tra_data8.txt",
   "../tra_data/tra_data9.txt",
 };

const char *test_files[2] = {
    "../tra_data/output_ori.csv",
    "../tra_data/output_can_traj.csv"
};

const char * filter_file_paths[FILE_NUM] = {
   "../tra_data/tra_data01filter.txt",
   "../tra_data/tra_data02filter.txt",
   "../tra_data/tra_data03filter.txt",
   "../tra_data/tra_data04filter.txt",
   "../tra_data/tra_data05filter.txt",
   "../tra_data/tra_data06filter.txt",
   "../tra_data/tra_data07filter.txt",
   "../tra_data/tra_data08filter.txt",
   "../tra_data/tra_data09filter.txt",
   "../tra_data/tra_data12filter.txt",
};
/*
   "../tra_data/tra_data19filter.txt",
   "../tra_data/tra_data21filter.txt",
   "../tra_data/tra_data22filter.txt",
   "../tra_data/tra_data23filter.txt",
   "../tra_data/tra_data24filter.txt",
   "../tra_data/tra_data25filter.txt",
   "../tra_data/tra_data26filter.txt",
   "../tra_data/tra_data27filter.txt",
   "../tra_data/tra_data28filter.txt",
   "../tra_data/tra_data29filter.txt",
};
*/
#endif
