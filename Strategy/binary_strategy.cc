...
void BinaryStrategy::FindCandidate(const GridPanel* grid_panel, const Trajectory& traj, double dis, list<int>& candidates) const {
...
void BinaryStrategy::GetCandidateTrajs(const GridPanel* grid_panel, const SamplePoint& point, double dis, list<int>& candidates) const {
...
void BinaryStrategy::TrajMergeJoin(const list<int>& fahter_trajs, const std::list<int>& child_trajs, list<int>& join_res) const {
	list<int>::iterator fa_itor, ch_itor;
	fa_itor = father_trajs.begin();
	ch_itor = child_trajs.begin();`
	while (fa_itor != father_trajs.end() && ch_itor != child_trajs.end()) {
		while (ch_itor != child_trajs.end()) {
			if (*fa_itor == *ch_itor) {
