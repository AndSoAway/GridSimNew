...
void MinDisStrategy::FindCandidateTrajs(const GridPanel* grid_panel, Trajectory& traj, double dis, std::list<int>& candidates) {
	traj.CalPointInfo(grid_panel);
	vector<PointInfo>& point_info = traj.PointInfo();
	...
	for (int i = 0; i < point_size; i++) {
		int traj_size = point_info[i].total_traj_count_; 
...
void MinDisStrategy::GetCandidateTrajs(const GridPanel* grid_panel, const PointInfo& point_info, list<int>& candidates, bool is_end) {
	const vector<pair<int, int>>& grid_index = point_info;	
	for(vector<pair<int, int>>::const_iterator itor = grid_index.cbegin(); itor != grid_index.cend(); itor++) {
		const list<int>& trajs = grid_panel->panel().GetTrajsInGrid(*itor);
